...
版本说明：仓库可核对的 Git Tag 为 `1.0.1` ～ `1.1.2`，当前分支描述为 `1.1.6`，工程版本号为 `1.1.6`（见 `LibmpvIptvClient.csproj` / `setup.iss`）。
...
| **精简模式** | 提供精简播放器窗口形态与独立交互 | 支持窗口态/全屏态同步，快捷键 W | 1.1.2 后（未打 Tag） |
...
| **EPG 节目单** | 支持 XMLTV (gz)，按日切换 | 自动匹配 `tvg-id`，快捷键 E | 1.0.1 起 |
| **回放 (Catchup)** | 支持基于模板的回放 URL 生成 | `{utc:yyyyMMddHHmmss}` 等 | 1.0.1 起 |
| **时移 (Time-Shift)** | 拖动进度条回看直播历史，快捷键 T | 依赖 `catchup-source` | 1.0.2 起 |
| **频道管理** | 分组、搜索、收藏、历史 | 收藏与历史本地持久化，快捷键 L/F | 分组/搜索/收藏：1.0.1 起；历史：1.0.4 起 |
...
| **录播与上传** | 本地录播、WebDAV 上传、上传队列 | 支持本地/远端双模式，快捷键 R | 1.1.2 后（未打 Tag） |
